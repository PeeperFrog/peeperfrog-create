...
def write_env_file(env_path, env_vars):
    """Write or update a .env file with the given variables.

    Preserves existing values that aren't being updated.
    """
    existing = {}
    ...
    with open(env_path, "w") as f:
        f.write("# PeeperFrog Create - API Keys and Credentials\n")
        f.write("# This file is auto-generated by setup.py\n")
...
def write_config_json(config_path, updates):
    """Update a config.json file with new values, preserving existing structure."""
    existing = {}
...
def add_servers_to_config(config_path, mcp_config, config_type="desktop"):
    ...
    Note: Credentials are NOT stored here - they go in .env files.
    This keeps secrets out of Claude's view.
    """
    ...
    # Merge in new servers - update command/args but preserve any existing env
    for server_name, server_config in mcp_config.get("mcpServers", {}).items():
        if server_name in existing["mcpServers"]:
            # Server already exists - update command/args, preserve env if any
            existing_server = existing["mcpServers"][server_name]
            existing_server["command"] = server_config["command"]
            existing_server["args"] = server_config["args"]
            # Keep existing env if present (for backwards compatibility)
        else:
...
def collect_wordpress_config():
    ...
    site_num = 1
    while True:
        url = input(f"  WordPress site {site_num} URL (e.g., https://example.com): ").strip()
        if not url:
            break
...
def offer_config_setup(install_dir, selected_servers, collected_keys=None):
    ...
    Credentials are stored in .env files in the codebase, keeping them
    out of Claude's view. The MCP config only contains paths.
    """
    if collected_keys is None:
        collected_keys = {}
    ...
    print("=" * 60)
    print("\n  Credentials are stored locally in the codebase (.env files),")
    print("  keeping them secure and out of Claude's view.")

    # Check for Claude Desktop config
    desktop_path = get_claude_desktop_config_path()
    code_path = get_claude_code_config_path()

    # Collect API keys and save to .env files
    if prompt_yes_no("\nWould you like to configure API keys now?"):
        for server_id in selected_servers:
            ...
            if keys:
                ...
                env_path = install_dir / server_id / ".env"
                write_env_file(env_path, keys)
    ...
    # Check .env files for missing required keys
    missing_keys = []
    for server_id in selected_servers:
        env_path = install_dir / server_id / ".env"
        ...
        for var in server_config.get("env_vars", []):
            if not existing_env.get(var):
                ...
                if info.get("required", False):
                    missing_keys.append((var, info, env_path))

    if missing_keys:
        print("\n  ⚠️  Required keys still need to be added:")
        for var, info, env_path in missing_keys:
